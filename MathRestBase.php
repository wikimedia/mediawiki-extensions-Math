 ...
 */
class MathRestBase extends MathRenderer {
	...
	*/
	...
	public function makeRequest( $post, &$res, &$error = '', &$hash = null ) {
		...
		$request = array(
			'method' => 'POST',
			//@TODO: figure out how to get the mapping stuff here without to copy code from VE
			'url' => 'http://localhost:7231/drmf-beta.wmflabs.org/v1/media/math/mml',
			'body' => $post
	...
	public function getPostData() {
		$input = $this->getTex();
		if ( $this->inputType == 'pmml' || $this->getMode() == 'latexml' && $this->getMathml() ) {
	...
	public function isValidMathML( $XML ) {
		...
		if ( !$xmlObject->wellFormed ) {
			LoggerFactory::getInstance( 'Math' )->error( 'XML validation error: ' .
														 var_export( $XML, true ) );
		} else {
	...
	private function getFallbackImageUrl( $noRender = false ) {
		...
		return "http://localhost:7231/drmf-beta.wmflabs.org/v1/media/math/svg/" . $this->hash;
	}
	...
	public function correctSvgStyle( $svg, &$style ) {
		if ( preg_match( '/style="([^"]*)"/', $svg, $styles ) ) {
			...
			if ( $this->getMathStyle() === 'display' ) {
				...
				$style =
					preg_replace( '/margin\-(left|right)\:\s*\d+(\%|in|cm|mm|em|ex|pt|pc|px)\;/',
						'', $style );
			}
	...
	private function getFallbackImage( $noRender = false, $classOverride = false ) {
		...
		return Xml::element( 'meta', $this->getAttributes( 'span', $attribs, array(
			'aria-hidden' => 'true',
			'style' => $style
		) ) );
	}
	...
	protected function getMathTableName() {
		throw new Exception( 'in restbase mode no database caching should happen' );
	}
	...
	public function getHtmlOutput() {
		...
		$mml = preg_replace( '/(<math[^>]*)(display|mode)=["\'](inline|block)["\']/', '$1',
			$this->getMathml() );
		if ( $this->getMathStyle() == 'display' ) {
