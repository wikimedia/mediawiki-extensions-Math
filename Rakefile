...
module MediawikiSelenium
  class RakeTask < Cucumber::Rake::Task
    def initialize(name: :selenium, test_dir: Environment.default_test_directory, site_tag: true, rerun: false)
      ...
      if rerun
        options += %x(cat #{workspace}/log/.cucumber.rerun)
      else
        options +=
          "#{Shellwords.escape(test_dir)}"\
...
#MediawikiSelenium::RakeTask.new(site_tag: false, rerun: false) # vagrant
