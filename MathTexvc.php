...
class MathTexvc extends MathRenderer {
	...
	public function callTexvc() {
		...
		$cmdArgs = [ $texvc, $tmpDir, $tmpDir, $this->getUserInputTex(), 'UTF-8', $wgTexvcBackgroundColor ];
		...
		if ( !isset( $_SERVER['PATH'] ) && $wgTexvcEnvPath === false ) {
			LoggerFactory::getInstance( 'Math' )->error(
				"PATH environment var is empty, set wgTexvcEnvPath so $texvc may find other required binaries." );
