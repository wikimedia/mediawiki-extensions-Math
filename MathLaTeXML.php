...
class MathLaTeXML extends MathRenderer {
	...
	*/
	 ...
	 * @param String $reqFactory class name of MWHttpRequest (needed for testing only)
	 * @return boolean success
	 */
	public function makeRequest( $host, $post, &$res, &$error = null, $reqFactory = 'MWHttpRequest') {
		global $wgLaTeXMLTimeout;
		$error = null;
		$res = null;
		$req = call_user_func(array($reqFactory,'factory'),$host
			, array( 'method' => 'POST', 'postData' => $post, 'timeout' => $wgLaTeXMLTimeout ));
		$status = $req->execute();
		if ( $status->isGood() ){
			$res = $req->getContent();
			return true;
