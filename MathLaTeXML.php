...
class MathLaTeXML extends MathRenderer {
	...
	*/
	 ...
	 */
	public function makeRequest( $host, $post, &$res, &$error = null, $httpRequestClass = 'MWHttpRequest' ) {
		...
		if ( $status->isGood() ) {
			...
			if ( $status->hasMessage( 'http-timed-out' ) ) {
				...
				$error = $this->getError( 'math_latexml_invalidresponse', $host ); // , $errormsg after change https://gerrit.wikimedia.org/r/#/c/64263/
				wfDebugLog( "Math", "\nLaTeXML NoResponse:"
					. var_export( array( 'post' => $post, 'host' => $host
