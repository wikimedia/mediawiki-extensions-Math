...
class MathLaTeXML extends MathMathML {
	...
	public function getLaTeXMLPostData() {
		...
		MathRenderer::LOG( 'Get post data: ' . $postData );
		return $postData;
	}
		...
		if ( $requestResult ) {
			...
			if ( $jsonResult && json_last_error() === JSON_ERROR_NONE ) {
				if ( $this->isValidMathML( $jsonResult->result ) ) {
					...
					$this->lastError =
						$this->getError( 'math_invalidxml', $this->getModeStr(), $host );
				...
				$this->lastError =
					$this->getError( 'math_invalidjson', $this->getModeStr(), $host );
	...
	public function calculateSvg() {
		...
		if ( $res == true ) {
			...
			MathRenderer::LOG( 'failed to convert LaTeXML-MathML to SVG:' . $lastError );
		}
		return $res;
	}
