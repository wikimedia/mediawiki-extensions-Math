	...
	*/
	...
	public function makeRequest( $host, $post, &$res, &$error = null ) {
		...
		if ( $wgLaTeXMLTimeout > 0 && $time > $wgLaTeXMLTimeout ) {
			...
			// In some cases the Http:post accepts result even if the time needed
			// is longer than the specified timeout. In order to guarantee a
			// deterministic behavior, we disregard these results.
			$res = false;
			wfDebugLog( "Math", "\nLaTeXML Timeout:"
		...
		if ( $res ) {
			...
			$error = $this->getError( 'math_latexml_invalidresponse', $host );
			wfDebugLog( "Math", "\nLaTeXML NoResponse:"
