		...
		if ( $requestResult ) {
			...
			if ( $jsonResult && json_last_error() === JSON_ERROR_NONE ) {
				if ( $jsonResult->success ) {
					// temporary fix to replace non well formed XML generated by MathJax
					if ( isset ( $jsonResult->mml ) ) {
						$mml = preg_replace( '/displaystyle="true" displaystyle="true"/' , 'displaystyle="true"' ,
							$jsonResult->mml );
					}
					if ( $this->getMode() == MW_MATH_LATEXML ||
							$this->inputType == 'pmml' ||
							$this->isValidMathML( $mml ) ) {
