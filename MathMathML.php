...
class MathMathML extends MathRenderer {
		...
		if ( $requestResult ) {
			...
			if ( $result && json_last_error() === JSON_ERROR_NONE ) {
				if ( $this->isValidMathML( $result->result ) ) {
					...
					$this->setPng( base64_decode( $result->png ) );
					return true;
				} else {
	 ...
	 * @param type $png if true png else svg in png mode the fallback style "display:none" will be added unless classOverride is set
	 * @param type $noRender if true no rendering will be performed if the image is not stored in the database
	 * @param type $classOverride if classOverride is false the class name will be calcuated by getClassName
	...
	public function getHtmlOutput() {
		global $wgMathDebug;
		$output = $wgMathDebug ? "\n<!--Math Block Beginn-->\n" : '';
		$overwriteClass = 'mwe-math-new-image';
		if ( $this->getMathml() ){
			...
			$output .= Xml::tags( 'span', array( 'class' => $class,
				'style'=>'display: none;' ),
				$this->getMathML() );
			$overwriteClass = false;
		}
		$output .= $this->getFallbackImage( false, false, $overwriteClass ) . "\n";
