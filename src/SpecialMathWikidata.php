	...
	public static function buildPageRepresentation(
		...
		if ( $info->getSymbol() ) {
			...
			$output->addWikiTextAsInterface(
				wfMessage( 'math-wikidata-formula' )->inContentLanguage() . "<math>$mathStr</math>"
			);
		}

		$output->addWikiTextAsInterface(
			wfMessage( 'math-wikidata-formula-name', $info->getLabel() )->inContentLanguage()
		);
		...
		$wikidataHeader = wfMessage(
			'math-wikidata-formula-link-header',
			$info->getDescription()
		)->inContentLanguage();
		$output->addWikiTextAsInterface( "== $wikidataHeader ==" );
