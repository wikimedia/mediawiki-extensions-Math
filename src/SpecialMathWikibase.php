...
class SpecialMathWikibase extends SpecialPage {
	...
	public static function buildPageRepresentation(
		...
		$header = wfMessage( 'math-wikibase-formula-information' )->inContentLanguage()->escaped();
		$output->addWikiTextAsInterface( "== $header ==" );

		if ( $info->getSymbol() ) {
			$mathStr = $info->getSymbol()->getValue();
			$labelFormula = wfMessage( 'math-wikibase-formula' )->inContentLanguage();
			$infoPattern = wfMessage(
				'math-wikibase-formula-header-format',
				$labelFormula
			)->inContentLanguage();

			// its necessary to replace math placeholder manually, otherwise a parse error occurrs
			$infoStr = str_replace( "$2", "<math>$mathStr</math>", $infoPattern );

			$output->addWikiTextAsInterface( $infoStr );
		}

		$labelName = wfMessage( 'math-wikibase-formula-name' )->inContentLanguage();
		...
		$labelType = wfMessage( 'math-wikibase-formula-type' )->inContentLanguage();
		$labelDesc = wfMessage( 'math-wikibase-formula-description' )->inContentLanguage();
		...
		if ( $info->hasParts() ) {
			...
			$output->addWikiTextAsInterface( $info->generateWikitextOfParts() );
		}
		...
		$output->addWikiTextAsInterface( "== $wikibaseHeader ==" );
		$output->addWikiTextAsInterface( MathWikibaseConnector::buildURL( $qid ) );
	}
