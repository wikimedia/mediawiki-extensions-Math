...
class MathMathML extends MathRenderer {
	...
	public function makeRequest(
		...
		if ( $status->isGood() ) {
			...
			if ( $status->hasMessage( 'http-timed-out' ) ) {
				...
				$res = false;
				$this->logger = LoggerFactory::getInstance( 'Math' );
				$logger->warning( 'Timeout:' . var_export( [
						'post' => $post,
						'host' => $host,
				...
				$error =
					$this->getError( 'math_invalidresponse', $this->getModeStr(), $host, $errormsg,
						$this->getModeStr() );
				$logger->warning( 'NoResponse:' . var_export( [
						'post' => $post,
						'host' => $host,
						'errormsg' => $errormsg
					], true ) );
	...
	protected function pickHost() {
		...
		$logger->debug( 'Picking host ' . $host );
		return $host;
	}
	...
	public function getPostData() {
		...
		$logger->debug( 'Get post data: ' . $out );
		return $out;
	}
	...
	public function isValidMathML( $XML ) {
		...
		if ( !$xmlObject->wellFormed ) {
			...
			if ( in_array( $localName, $this->getAllowedRootElements() ) ) {
				...
				$logger->error( "Got wrong root element: $name" );
			}
		}
		return $out;
	}
	...
	protected function isEmpty() {
		if ( $this->userInputTex === '' ) {
			$logger->debug( 'Rendering was requested, but no TeX string is specified.' );
