...
class MathWikibaseInfo {
	...
	public function generateWikitextOfParts() {
		...
		foreach ( $this->hasParts as $part ) {
			$label = $this->escapePipesInTable( $part->getLabel() );
			$output .= "\n| style='text-align:$labelAlign;' | '''$label'''";

			$output .= "\n| ";
			if ( $part->getSymbol() ) {
				$formula = $part->getSymbol()->getValue();
				$output .= "style='text-align:center; padding: 2px; " .
					"padding-left: 10px; padding-right: 10px;' | <math> $formula </math>";
			}

			if ( $part->getDescription() ) {
				$desc = $this->escapePipesInTable( $part->getDescription() );
				$output .= "\n| style='text-align:$labelAlignOpposite;' | ''$desc''";
			} else {
				$output .= "\n|";
			}
...
// $output .= ": ";
			...
			$output .= Html::openElement( "i" ) . $part->getLabel() . Html::closeElement( "i" );
...
// $output .= ;
	 ...
	 * @return mixed
	 */
	private function escapePipesInTable( $string ) {
		return str_replace( '|', '<nowiki>|</nowiki>', $string );
	}
}
