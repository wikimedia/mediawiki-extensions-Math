...
class MathInputCheckTexvc extends MathInputCheck {
	...
	public function convertTexvcError($texvcResult) {
		...
		switch ($texvcStatus) {
			case 'E':
				$errMsg = MathRenderer::getError('math_lexing_error');
				break;
			case 'S':
				$errMsg = MathRenderer::getError('math_syntax_error');
				break;
			case 'F':
				$errMsg = MathRenderer::getError('math_unknown_function', $errDetails);
				break;
			default:
				$errMsg = MathRenderer::getError('math_unknown_error');
		}
