( function ( $ ) {
	...
	var oprversion = Number( RegExp.$1 );
	...
	// Always do png fallback on opera 9 due to T137765 but also make sure we doint hit opera 10 with this
	// hack.
	if ( /Opera[\/\s](\d+\.\d+)/.test( navigator.userAgent ) ) {
		var oprversion = Number( RegExp.$1 );
		if ( oprversion === 9 && oprversion !== 10 ) {
	...
	} else if ( !document.implementation.hasFeature( 'http://www.w3.org/TR/SVG11/feature#Image', '1.1' ) ) {
		$( '.mwe-math-fallback-image-inline, .mwe-math-fallback-image-display' ).each( function ( ) {
			this.src = this.src.replace( 'media/math/render/svg/', 'media/math/render/png/' );
		} );
	}
