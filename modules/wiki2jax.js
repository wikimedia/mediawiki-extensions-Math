 ...
 */
 /*global MathJax:true, data:true, mw:true */
MathJax.Extension.wiki2jax = {
  ...
  PreProcess: function (element) {
    ...
    var that = this;
    document.getElementById('.mwe-math-fallback-png-display, .mwe-math-fallback-png-inline, .mwe-math-fallback-source-display,' +
  ...
  ConvertMath: function (node) {
	...
	if (node.nodeName === 'IMG') {
		tex = node.alt;
          ...
          if (node.nodeName === 'STRONG') {
            tex = document.getElementById(node).text().replace(/^[^:]*: (.*)$/,'$1');
          } else {
            tex = document.getElementById(node).text().replace(/^\$/,'').replace(/\$$/,'');
          }
          tex = tex.replace(/&lt;/g,'<').replace(/&gt;/g,'>').replace(/&amp;/g,'&').replace(/&nbsp;/g,' ');
	}
    if ( document.getElementById( node ).hasClass( 'mwe-math-fallback-png-display') || document.getElementById( node ).hasClass( 'mwe-math-fallback-source-display') ) {
      mode = '; mode=display';
    }
    ...
    if (mode === '' && parent.firstChild === node) {
    mode = '; mode=display-nobreak';
    }
...
MathJax.Ajax.loadComplete(mw.config.get( 'wgServer' ) + mw.config.get( 'wgExtensionAssetsPath' ) + '/Math/modules/wiki2jax.js');