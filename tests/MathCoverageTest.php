 ...
 * The testData is generated by the page SpecialPage MathDebug that is provided by the MathSearch
 * extension. To update the test data locally with vagrant the following procedure is recommended:
 *
 * 1. Enable the role mathsearch and run vagrant provision
 * 2. copy the source from https://www.mediawiki.org/wiki/Extension:Math/CoverageTest to a new page e.g.
 *    MathTest at your local vagrant instance
 * 3. Open http://127.0.0.1:8080/wiki/Special:MathDebug?length=500&offset=0&page=MathTest&action=generateParserTests
 * 4. This will offer you a download of a file that contains test for the first 500 equations of the CoverageTest page
 * 5. Save that file to /vagrant/mediawiki/extensions/Math/tests/ParserTest.data
 * 6. Test local e.g. via
