...
class MathoidParserTest extends MediaWikiTestCase {
  ...
  <semantics>
	...
	/**
	 * Checks error behaviour functionality of mathoid (running on http://localhost:10042)
	 * @test
	 */
	public function testError() {
		$tex = '\\badcommand x';
		$requestResult = $this->makeRequest( 'http://localhost:10042', 'q=' . rawurlencode( $tex ) );
		$res = $this->decodeJson( $requestResult );
		$this->assertFalse( $res->success, 'Bad command did not lead to an error: ' . var_export( $res, true ) );
	}
	/**
