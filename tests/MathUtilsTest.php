<?php
/**
 * Test the MathUtilities output format.
 *
 * @group Math
 */
class MathUtilsTest extends MediaWikiTestCase {
	...
	public function testMathStyleToString() {
		$default = 'inlineDisplaystyle';
		$testCases = array(
			'MW_MATHSTYLE_INLINE_DISPLAYSTYLE'  => 'inlineDisplaystyle', //default large operator inline
			'MW_MATHSTYLE_DISPLAY'              => 'display', // large operators centered in a new line
			'MW_MATHSTYLE_INLINE'               => 'inline', // small operators inline
			0                                   => 'inlineDisplaystyle', //default large operator inline
			1                                   => 'display', // large operators centered in a new line
			2                                   => 'inline', // small operators inline
		);
		foreach ( $testCases as $input => $expected ){
	...
	public function testMathCheckToString() {
		...
		foreach ( $testCases as $input => $expected ){
			...
			$this->assertEquals( $expected, $real, "Convertion MathML mode" );
		}
	}
