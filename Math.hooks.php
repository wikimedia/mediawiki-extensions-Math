...
class MathHooks {
	 ...
	 *
	 * @param SpecialPage $searchPage
	 * @param string $extract the text snipped going to be displayed
	 * @return bool
	 */
	static function onShowSearchHit( SpecialPage $searchPage, $result, $terms, &$link, &$redirect, &$section,
		...
		foreach ( $mathTags as $id => $tag ) {
			...
			if ( preg_match( '#</\s*math\s*>$#', $fullElement) ) { // only full elements
				...
				// reverse effects from htmlspecialchars
		...
		// Add modules required for displaying math
		if( sizeof( $mathTags ) > 0  ){
			$out = $searchPage->getOutput();
			$out->addModuleStyles( array( 'ext.math.styles', 'ext.math.desktop.styles' ) );
	...
	private static function unEscapeHtmlTags( $in ){
		...
		$out = str_replace( '<', '-lt-' . self::$random . Parser::MARKER_SUFFIX, $out );
		$out = str_replace( '>', '-gt-' . self::$random . Parser::MARKER_SUFFIX, $out );
